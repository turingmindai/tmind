---
allowed-tools: Bash(git diff:*), Bash(git status:*), Bash(git log:*), Bash(git blame:*), Bash(git show:*), Read
description: Quick code review for uncommitted local changes (Local Mode)
---

Quick code review for uncommitted changes. Fast, focused on critical issues.
Running in **Local Mode** (no API key detected).

## Step 0: Local Initialization

```bash
echo "ℹ️  Running in local-only mode (implicit)"
# No cloud memory context fetching
```
